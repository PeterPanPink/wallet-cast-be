"""
Mock implementation of the cbx-live service endpoints.

This FastAPI app exposes pared-down versions of the upstream routes so the CW
...
Run with granian:
    granian --interface ASGI --host 127.0.0.1 --port 18081 tools.mock_cbx_live:app

Then point CBX_LIVE_API_URL to http://127.0.0.1:18081 (e.g. in env.local / env.example).
"""
...
app = FastAPI(title="cbx-live mock", version="0.1.0")
...
async def health():
    ...
    return {"status": "ok", "service": "mock-cbx-live"}
