...
from app.services.integrations.external_live.external_live_schemas import (
    ...
    AdminStopLiveBody,
    AdminUpdateLiveBody,
    ExternalLiveApiResponse,
)


class ExternalLiveClient:
    ...
    async def admin_start_live(self, body: AdminStartLiveBody) -> ExternalLiveApiResponse:
        """Call Admin Start Live endpoint."""
        demo_mode = str(config.get("DEMO_MODE", "true")).strip().lower() == "true"
        if demo_mode:
            logger.info("External Live client DEMO_MODE=true: returning stubbed start_live response")
            # Minimal, demo-safe response shape
            return ExternalLiveApiResponse.model_validate(
                {
                    "success": True,
        ...
        async with httpx.AsyncClient() as client:
            ...
            try:
                return ExternalLiveApiResponse.model_validate(data)
            except Exception:
    ...
    async def admin_stop_live(self, body: AdminStopLiveBody) -> None:
        ...
        demo_mode = str(config.get("DEMO_MODE", "true")).strip().lower() == "true"
        if demo_mode:
            logger.info("External Live client DEMO_MODE=true: stubbed stop_live (no-op)")
            return
    ...
    async def admin_update_live(self, body: AdminUpdateLiveBody) -> None:
        ...
        demo_mode = str(config.get("DEMO_MODE", "true")).strip().lower() == "true"
        if demo_mode:
            logger.info("External Live client DEMO_MODE=true: stubbed update_live (no-op)")
            return
        ...
        async with httpx.AsyncClient() as client:
            ...
            try:
                ...
                if errcode:
                    logger.error(f"External Live admin/live/update error: {errcode} - {errmesg}")
            except Exception:
