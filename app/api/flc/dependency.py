...
from app.cw.api.cbe.auth.verify_token import verify_token
from app.cw.api.utils import get_redis_major_client
...
async def get_current_user(
    ...
    logger.debug("get_current_user headers: {}", dict(request.headers))
    user_info = await verify_token(request, redis_client)
    if not user_info:
        raise FlcError(
            errcode=FlcErrorCode.E_BAD_TOKEN,
            errmesg="Invalid token",
            status_code=FlcStatusCode.UNAUTHORIZED,
        )
    ...
    if not await check_user_in_whitelist(redis_client, user_id):
        raise FlcError(
            errcode=FlcErrorCode.E_NOT_WHITELISTED,
