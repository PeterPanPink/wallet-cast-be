...
from app.api.flc.schemas.session_egress import GetTranscriptsOut, TranscriptItem
...
router = APIRouter(prefix="/flc/session/egress/caption", tags=["Next Phase"])


@router.get("/{session_id}/transcripts", response_model=CwOut[GetTranscriptsOut])
async def get_transcripts(
    ...
    return CwOut[GetTranscriptsOut](
...
async def get_captions_vtt(
    ...
    if not session.started_at:
        raise FlcError(
            errcode=FlcErrorCode.E_SESSION_NOT_STARTED,
...
async def get_master_playlist(
    ...
    if not mux_playback_ids or len(mux_playback_ids) == 0:
        raise FlcError(
            errcode=FlcErrorCode.E_NO_PLAYBACK_ID,
    ...
    if not playback_id:
        raise FlcError(
            errcode=FlcErrorCode.E_NO_PUBLIC_PLAYBACK_ID,
