...
from app.api.v1.schemas.session_egress import UpdateRoomMetadataIn, UpdateRoomMetadataOut
...
router = APIRouter(prefix="/session/ingress")
...
async def create_room(
    ...
    if session.status != SessionState.READY and not SessionStateMachine.can_transition(
        ...
        raise AppError(
            errcode=AppErrorCode.E_SESSION_TERMINATED,
...
async def get_host_token(
    ...
    if not livekit_url:
        raise AppError(
            errcode=AppErrorCode.E_LIVEKIT_NOT_CONFIGURED,
            errmesg="LIVEKIT_URL is not configured",
            status_code=HttpStatusCode.INTERNAL_SERVER_ERROR,
        )

    return ApiOut[AccessTokenOut](
...
async def update_room_metadata(
    ...
    return ApiOut[UpdateRoomMetadataOut](
...
async def update_participant_name(
    ...
    service: SessionService = Depends(get_session_service),
) -> ApiOut[UpdateParticipantNameOut]:
    ...
    return ApiOut[UpdateParticipantNameOut](
...
async def get_invite_link(
    ...
    return ApiOut[GetInviteLinkOut](results=GetInviteLinkOut(invite_link=invite_link))
