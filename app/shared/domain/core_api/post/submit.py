...
async def submit(request: Request) -> Tuple[bool, SubmitResult | SubmitError, int | None]:
    """Call core-api /u/post to create a post and return the response."""

    logger.debug('core-api post submit: path={} method={}', request.url.path, request.method)
    ...
    try:
        ...
        logger.warning('core-api post submit invalid payload: {}', exc)
        return False, {
    ...
    try:
        async with httpx.AsyncClient(http2=True, timeout=timeout) as client:
            logger.debug('calling core-api post submit url={}', url)
            response = await client.post(url, json=payload, headers=headers)
    except Exception as exc:  # noqa: BLE001 - surface upstream
        logger.warning('core-api post submit request failed: {}', exc)
        return False, {
    ...
    logger.debug('core-api post submit status={} headers={}', status_code, dict(response.headers))

    try:
        data = response.json()
    except Exception:
        text = response.text.strip()
        logger.warning('core-api post submit invalid json status={} text={}', status_code, text)
        return False, {
            'errcode': 'E_POST_SUBMIT_FAILED',
            'errmesg': text or 'Invalid response from core-api',
        }, status_code if status_code >= 400 else 502

    logger.debug('core-api post submit body={}', data)
