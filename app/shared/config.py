...
class CustomConfig:
    ...
    - Searches parent directories for shared-lib.yml starting from project root
    ...
    def _find_yaml(self):
        try:
            ...
            for base in [start] + list(start.parents):
                candidate = base / 'shared-lib.yml'
                if candidate.exists() and candidate.is_file():
                    logger.info("Loaded custom config from {}", candidate)
                    return candidate
        except Exception as e:
            logger.warning("Failed searching shared-lib.yml: {}", e)
        return None
    ...
    def _load_yaml(self, path: Path):
        try:
            ...
            logger.warning("PyYAML not installed; skipping shared-lib.yml load")
            return {}
        except Exception as e:
    ...
    def get_dynamic_config_root_key(self) -> str:
        return self._data.get('dynamic_config_root_key', 'shared-lib:config')
