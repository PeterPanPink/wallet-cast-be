...
class CorebeError(ApiResponse):
    rc: str = 'ERR'
    error: dict


class CorebeResult(ApiResponse):
    rc: str = 'OK'
    result: Any
...
def corebe_error(failure: ApiFailure | dict):
...
def corebe_result(success: Any):
    if isinstance(success, ApiSuccess):
        if isinstance(success.results, BaseModel):
            return CorebeResult(result=success.results.model_dump())
        else:
            return CorebeResult(result=success.results)
    elif isinstance(success, BaseModel):
        return CorebeResult(result=success.model_dump())
    else:
        return CorebeResult(result=success)


def is_corebe_request(request: Request):
    is_corebe = request.query_params.get('is_corebe') or 'true'
    return is_corebe.lower() in ('true', 'yes', 'on', '1')
...
def make_response(results, is_corebe: bool, *, status_code: int = None):
    if isinstance(results, Exception):
        failure = api_failure(errmesg=format_error(results))
        response = corebe_error(failure) if is_corebe else failure
        if status_code is None:
            status_code = 500
    else:
        is_error, is_internal = check_error(results)
        if is_error:
            response = corebe_error(results) if is_corebe else results
            if status_code is None:
                status_code = 500 if is_internal else 400
        else:
            response = corebe_result(results) if is_corebe else results
            if status_code is None:
                status_code = 200
        ...
        if x.name.endswith('.py') and x.name != '__init__.py':
            ...
            try:
                ...
                if name.startswith('...api.'):
                    name = name.replace('...api.', f'{app_module_prefix}cw.api.', 1)
