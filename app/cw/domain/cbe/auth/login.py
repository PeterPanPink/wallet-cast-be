...
async def login(request: Request) -> Tuple[bool, LoginResult | LoginError, int | None]:
    """Call core-be login API and sanitize the response."""

    logger.debug('core-be login: path={} method={}', request.url.path, request.method)
    ...
    try:
        ...
        logger.warning('core-be login invalid payload: {}', exc)
        return False, {
    ...
    try:
        async with httpx.AsyncClient(http2=True, timeout=timeout) as client:
            logger.debug('calling core-be login url={}', url)
            response = await client.post(url, json=payload, headers=headers)
    except Exception as exc:  # noqa: BLE001 - surface upstream
        logger.warning('core-be login request failed: {}', exc)
        return False, {
    ...
    logger.debug('core-be login status={} headers={}', status_code, dict(response.headers))

    try:
        data = response.json()
    except Exception:
        text = response.text.strip()
        logger.warning('core-be login invalid json status={} text={}', status_code, text)
        return False, {
            'errcode': 'E_AUTH_LOGIN_FAILED',
            'errmesg': text or 'Invalid response from core-be',
        }, status_code if status_code >= 400 else 502

    logger.debug('core-be login body={}', data)

    if not isinstance(data, dict):
        return False, {
            'errcode': 'E_AUTH_LOGIN_FAILED',
            'errmesg': 'Unexpected response from core-be',
        }, status_code if status_code >= 400 else 502
    ...
    if status_code != 200 or data.get('rc') != 'OK':
        ...
        logger.debug('core-be login failure errcode={} errmesg={}', errcode, errmesg)
    ...
    if not isinstance(result, dict):
        return False, {
            'errcode': 'E_AUTH_LOGIN_FAILED',
            'errmesg': 'Malformed result from core-be',
        }, 502
    ...
    logger.debug('core-be login success user_keys={} result_keys={}',
