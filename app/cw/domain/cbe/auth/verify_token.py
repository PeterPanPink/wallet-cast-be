...
async def verify_token(request: Request, redis_client: Redis) -> Optional[Dict[str, Any]]:
    ...
    async with lock:
        ...
        if not base_url:
            logger.warning('COREBE_API_URL not configured')
            return None
        ...
        for attempt in (1, 2):
            try:
                async with httpx.AsyncClient(http2=True, timeout=timeout) as client:
                    logger.debug('calling core-be attempt={} url={}', attempt, url)
                    resp = await client.get(url, headers=headers)
                    if resp.status_code == 200:
                        body = resp.json()
                        logger.debug('core-be response body={}', body)
                        ok = isinstance(body, dict) and body.get('rc') == 'OK' and body.get('result') == 'valid'
                    elif resp.status_code == 401:
                        logger.debug('core-be response 401 body={}', resp.json())
                        ok = False
                    elif resp.status_code == 429:
                        logger.debug('core-be response 429 body={}', resp.json())
                        ok = False
                    else:
                        logger.debug('core-be response status={} text={}', resp.status_code, resp.text)
                        ok = False
                break
                ...
                if attempt == 1:
                    ...
                    continue
                logger.warning('call core-be failed: {}', e)
                ok = False
