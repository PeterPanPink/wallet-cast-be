...
CW Library is designed to be embedded as a submodule in other projects, providing essential backend functionality including:
...
git subtree add --prefix=app/cw <redacted-remote-url> library-v0.1.0 --squash
```
...
In your Python code, you can now import and use the cw library:

```python
# Import API utilities
from app.cw.api.admin import some_admin_function
from app.cw.api.health import health_check
from app.cw.api.utils import some_utility_function

# Import storage managers
from app.cw.storage.redis import RedisManager, get_cache_client
from app.cw.storage.mongo import MongoManager, get_mongo_client
...
To update the cw library to a newer version:

```bash
# Update to a specific new version
git subtree pull --prefix=app/cw <redacted-remote-url> library-v0.2.0 --squash
```

### Step 5: Configuration

cw-lib supports application-level configuration via an optional `cw-lib.yml` file (and still supports demo env files).
...
#### Optional YAML file (cw-lib.yml)

Place `cw-lib.yml` in the project root or a parent directory (CustomConfig searches upward):
...
dynamic_config_root_key: cw:config
...
- When using the library branch (recommended via git subtree to `app/cw`): the example is at `app/cw/cw-lib-example.yml`. Copy it to the same level as `cw` and rename to `cw-lib.yml` (i.e., `app/cw-lib.yml`).

- When using the release branch (full project): the example is at `src/cw-lib-example.yml`. Copy it to the same level as `cw` and rename to `cw-lib.yml` (for example, if `cw` is at `app/cw`, copy to `app/cw-lib.yml`).
...
1. **Clean Import Path**: `from cw.api import ...` provides intuitive namespace
2. **Namespace Isolation**: The `cw` namespace prevents conflicts with other libraries
