...
from app.api.flc.routers.session_ingress_caption import (
    get_caption_worker,
    get_session_service,
    router,
)
...
class TestEnableCaption:
    """Tests for POST /flc/session/ingress/caption/enable endpoint."""
    ...
    def test_enable_caption_success(
        ...
        with patch("app.api.flc.routers.session_ingress_caption.start_caption_agent") as mock_start:
            ...
            # Act
            response = client.post("/flc/session/ingress/enable_caption", json=payload)
...
class TestDisableCaption:
    """Tests for POST /flc/session/ingress/caption/disable endpoint."""
    ...
    def test_disable_caption_success(
        ...
        mock_task = MagicMock()
        mock_task.id = "test_job_123"
        with patch("app.api.flc.routers.session_ingress_caption.stop_caption_agent") as mock_stop:
            ...
            # Act
            response = client.post("/flc/session/ingress/disable_caption", json=payload)
...
class TestGetCaptionStatus:
    """Tests for POST /flc/session/ingress/caption/status endpoint."""
    ...
    def test_get_caption_status_enabled(
        ...
        # Act
        with patch(
            "app.api.flc.routers.session_ingress_caption.get_redis_client"
        ) as mock_get_redis:
            ...
            response = client.post("/flc/session/ingress/get_caption_status", json=payload)
...
class TestUpdateParticipantLanguage:
    """Tests for POST /flc/session/ingress/caption/update-language endpoint."""
    ...
    def test_update_language_success(
        ...
        # Act
        with patch("app.services.cw_livekit.livekit_service") as mock_livekit:
            mock_livekit.update_participant = AsyncMock()

            response = client.post("/flc/session/ingress/update-language", json=payload)
    ...
    def test_update_language_livekit_error(
        ...
        # Act - The service layer catches exceptions and raises FlcError
        with patch(
            "app.services.cw_livekit.livekit_service.update_participant",
            new_callable=AsyncMock,
            side_effect=FlcError(
                errcode=FlcErrorCode.E_PARTICIPANT_LANGUAGE_UPDATE_FAILED,
