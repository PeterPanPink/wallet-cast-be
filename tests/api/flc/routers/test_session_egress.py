...
from app.api.flc.routers.session_egress import get_session_service, router
...
class TestStartLiveStream:
    """Tests for POST /flc/session/egress/start_live_stream endpoint."""
    ...
    def test_start_live_stream_with_session_id_success(
        ...
        # Act
        response = client.post("/flc/session/egress/start_live_stream", json=payload)
    ...
    def test_start_live_stream_with_referer_header(
        ...
        response = client.post(
            "/flc/session/egress/start_live_stream",
            json=payload,
    ...
    def test_start_live_stream_already_exists_no_data(
        ...
        # Arrange
        mock_session_service.get_session.return_value = mock_session_response
        mock_session_service.start_live.side_effect = FlcError(
    ...
    def test_start_live_stream_flc_error(
        ...
        """Should propagate FlcError from service layer."""
        # Arrange
        mock_session_service.get_session.return_value = mock_session_response
        mock_session_service.start_live.side_effect = FlcError(
            errcode=FlcErrorCode.E_LIVE_STREAM_START_FAILED,
...
class TestEndLiveStream:
    """Tests for POST /flc/session/egress/end_live_stream endpoint."""
    ...
    def test_end_live_stream_with_session_id_success(
        ...
        # Act
        response = client.post("/flc/session/egress/end_live_stream", json=payload)
    ...
    def test_end_live_stream_value_error(
        ...
        mock_session_service.get_session.return_value = mock_session_response
        mock_session_service.end_live.side_effect = FlcError(
